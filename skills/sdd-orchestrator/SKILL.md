...
| Mode | Description |
|------|-------------|
| `engram` | Fast, no files. Best for solo work. Re-running a phase overwrites (no history). |
| `openspec` | File-based in `openspec/`. Committable, full git history. |
| `hybrid` | Both — team sharing + cross-session recovery. Higher token cost. |

If the user doesn't specify: engram is available → default `engram`. Otherwise → `none`.
...
- Skills: orchestrator resolves compact rules from the registry and injects them as `## Project Standards (auto-resolved)` in the sub-agent prompt.
