...
**Artifacts**: Engram `sdd/{change-name}/proposal` | `.king/sdd/{change-name}/proposal.md`
...
- El budget default de PR review es **400 líneas cambiadas** (`additions + deletions`).
- El orquestador DEBE cachear una delivery strategy al inicio de sesión: `ask-on-risk` (default), `auto-chain`, `single-pr`, o `exception-ok`.
- El orquestador DEBE pasar `delivery_strategy` a `sdd-tasks` y la decisión resuelta a `sdd-apply`.
- `sdd-tasks` DEBE hacer un forecast de si el trabajo planificado puede superar ese budget.
- El forecast DEBE incluir exactamente estas líneas en texto plano: `Decision needed before apply: Yes|No`, `Chained PRs recommended: Yes|No`, y `400-line budget risk: Low|Medium|High`.
- Si el forecast es High, `sdd-tasks` DEBE recomendar PRs encadenados o stacked usando work units entregables.
- `sdd-apply` NO DEBE comenzar trabajo oversized salvo que la delivery strategy resuelva en slices encadenados/stacked o se haya aceptado explícitamente `size:exception`.
- Cada slice de PR encadenado debe tener un inicio claro, fin claro, scope autónomo, verificación, y rollback razonable.
- En Feature Branch Chain, PR #1 apunta al branch feature/tracker y PRs posteriores apuntan al PR inmediatamente anterior; si GitHub muestra slices previos en el diff de un PR hijo, retargetear/rebasear hasta que el diff sea limpio.

Este guard existe para reducir el burnout del reviewer y mantener la entrega de implementación segura. No tratarlo como ruido de proceso opcional.
```
